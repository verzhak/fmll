...
\ru
	...
	-# Инициализировать матрицу смежности нейронов сети, найти входные и выходные нейроны сети с помощью функции fmll_ff_set_connect();
	-# Иницилизировать веса синапсов нейронов сети с помощью одной из *_ff_weight_init_* функций;
	-# TODO Обучить сеть с помощью одной из *_ff_teach_* функций;
...
typedef struct t_fmll_ff
{
	...
	/*! \en TRANSLATE \ru Массив функций активаций (своя функция активации для каждого нейрона) \endlang */
	double (** fun)(double);
...
\en
	...
	\param ff - TRANSLATE;
	\param connect - TRANSLATE (TRANSLATE); TRANSLATE.
...
\ru
	...
	\brief Инициализация матрицы смежности нейронов сети прямого распространения, поиск входных и выходных нейронов сети

	\param ff - указатель на описатель сети;
	\param connect - матрица смежности нейронов сети (элемент [v][u] данной матрицы устанавливается в 1, если существует синапс из нейрона v в нейрон u, и в 0 в противном случае); матрица connect не должна содержать циклов.

	\return 0 - в случае успешного завершения операций инициализации матрицы смежности и поиска входных и выходных нейронов сети;
	\return <> 0 - в случае некорректного завершения операций инициализации матрицы смежности и поиска входных и выходных нейронов сети.
